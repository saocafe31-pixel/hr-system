-- Web Push pipeline for PWA clients
...
create index if not exists web_push_jobs_status_next_idx
  on public.web_push_notification_jobs (status, next_attempt_at, created_at);

alter table public.web_push_notification_jobs enable row level security;

drop policy if exists "web_push_jobs_deny_all_select" on public.web_push_notification_jobs;
create policy "web_push_jobs_deny_all_select" on public.web_push_notification_jobs
  for select to authenticated
  using (false);

drop policy if exists "web_push_jobs_deny_all_insert" on public.web_push_notification_jobs;
create policy "web_push_jobs_deny_all_insert" on public.web_push_notification_jobs
  for insert to authenticated
  with check (false);

drop policy if exists "web_push_jobs_deny_all_update" on public.web_push_notification_jobs;
create policy "web_push_jobs_deny_all_update" on public.web_push_notification_jobs
  for update to authenticated
  using (false)
  with check (false);
...
create or replace function public.finalize_web_push_notification_job(
  p_id uuid,
  p_ok boolean,
...
begin
  ...
  set
    ...
    last_error = left(coalesce(p_error, 'web_push_failed'), 1000)
  where id = p_id;
end;
$$;

create or replace function public.enqueue_web_push_on_task_notification()
returns trigger
language plpgsql
security definer
set search_path = public
...
create or replace function public.enqueue_web_push_on_chat_mention()
returns trigger
language plpgsql
security definer
set search_path = public
...
create or replace function public.enqueue_web_push_on_post_comment()
returns trigger
language plpgsql
security definer
set search_path = public
...
create or replace function public.enqueue_web_push_on_note_reply()
returns trigger
language plpgsql
security definer
set search_path = public
...
drop trigger if exists trg_web_push_task_notifications on public.task_notifications;
create trigger trg_web_push_task_notifications
after insert on public.task_notifications
for each row execute function public.enqueue_web_push_on_task_notification();

drop trigger if exists trg_web_push_chat_mentions on public.attendance_chat_mention_notifications;
create trigger trg_web_push_chat_mentions
after insert on public.attendance_chat_mention_notifications
for each row execute function public.enqueue_web_push_on_chat_mention();

drop trigger if exists trg_web_push_post_comments on public.community_feed_comments;
create trigger trg_web_push_post_comments
after insert on public.community_feed_comments
for each row execute function public.enqueue_web_push_on_post_comment();

drop trigger if exists trg_web_push_note_replies on public.community_note_replies;
create trigger trg_web_push_note_replies
after insert on public.community_note_replies
for each row execute function public.enqueue_web_push_on_note_reply();
