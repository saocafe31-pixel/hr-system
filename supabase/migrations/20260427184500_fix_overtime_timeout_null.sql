-- Fix OT processor: ensure timeout_minutes never becomes NULL
-- when app_settings row is missing or malformed.
...
begin
  ...
  -- critical fallback: SELECT INTO can return NULL when no row found
