...
| `manager` | ทุกอย่างของ employee + **ตารางงาน**, **ทีม**, มอบหมายงาน — **ทีม / อนุมัติลา / จัดกะ / วันหยุดลูกทีม** ตาม `manager_direct_reports` ที่แอดมินกำหนด (สิทธิ์เปิดอัตโนมัติเมื่อมีลูกทีม) |
...
| `employee_holiday_dates` | วันหยุดตามวันที่จริงต่อพนักงาน (`holiday_date`) — หน้า `schedule` ตั้งผ่านปุ่ม `วันหยุด` (เลือกหลายวันที่/หลายคน ไม่ซ้ำทุกสัปดาห์); รายละเอียดวันในปฏิทินแสดงรายชื่อหยุดรวมตามสาขา — RLS: แอดมิน / ผู้จัดการที่เป็น `manager_direct_reports` ของพนักงาน |
...
| `attendance_calendar_notes` | โน้ตและเช็กลิสต์รายวันในปฏิทิน (หน้า attendance + ปฏิทินในโปรไฟล์/ทีม/แชท) — RLS select: เจ้าของ; แอดมิน; ผู้จัดการอ่านลูกทีมได้; ผู้ใช้ที่ไม่ใช่แอดมินและ `same_branch_as` กับเจ้าของโน้ตอ่านได้ (อ่านอย่างเดียว); แก้ไขแทนลูกทีมได้เมื่อเป็น `manager_direct_reports`; แอดมินเต็มสิทธิ์ |
...
|---------------------|------|
| 2026-06-24 | RLS วันหยุด: ผู้จัดการตั้งวันหยุดให้ตัวเองได้ (manager_may_manage_subordinate รวม auth.uid()) |
| 2026-06-24 | ลบส่วนรหัสผ่าน legacy ออกจากโมดัลแก้ไขพนักงาน — ใช้เฉพาะรหัสล็อกอิน (แอป) |
| 2026-06-24 | แอดมินตั้งรหัสล็อกอินใหม่ (Edge admin-reset-user-password) + RPC legacy password ไม่พึ่งคอลัมน์ Password ใน PostgREST |
| 2026-06-24 | Manager + ลูกทีม: RLS วันหยุด/มอบหมายกะ/อนุมัติลา ตาม `manager_direct_reports` (ไม่ต้องเปิดสวิตช์แยก) · backfill สิทธิ์ · แอปกรองทีมจากลูกทีม |
| 2026-06-24 | Payroll รายวัน/ชม.: นับวันมอบหมายรวมวันหยุดที่มอบหมาย · ชม.จากเวลาเข้า-ออกหักพักตรงตารางทีม · รอบ 26–25 สอดคล้อง cycle key |
